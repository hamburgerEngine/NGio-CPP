#include "io/newgrounds/crypto/AES.hpp"
#include <cstring>

namespace io {
namespace newgrounds {
namespace crypto {

static const uint8_t sbox[256] = {
    
};

static const uint8_t Rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

AESImpl::AESImpl(const std::vector<uint8_t>& key_data) {
    std::copy(key_data.begin(), key_data.begin() + KEY_SIZE, key.begin());
    keyExpansion();
}

void AESImpl::keyExpansion() {
    uint32_t temp;
    int i = 0;

    for(; i < 4; i++) {
        roundKey[i] = (key[4*i] << 24) | (key[4*i+1] << 16) | (key[4*i+2] << 8) | key[4*i+3];
    }

    while(i < 4 * (NR + 1)) {
        temp = roundKey[i-1];
        if(i % 4 == 0) {
            temp = ((sbox[(temp >> 16) & 0xFF] << 24) |
                   (sbox[(temp >> 8) & 0xFF] << 16) |
                   (sbox[temp & 0xFF] << 8) |
                   sbox[(temp >> 24)]) ^ (Rcon[i/4] << 24);
        }
        roundKey[i] = roundKey[i-4] ^ temp;
        i++;
    }
}

void AESImpl::addRoundKey(uint8_t state[4][4], const uint32_t* roundKey) {
    for(int i = 0; i < 4; i++) {
        uint32_t word = roundKey[i];
        for(int j = 0; j < 4; j++) {
            state[j][i] ^= (word >> (24 - 8*j)) & 0xFF;
        }
    }
}

void AESImpl::subBytes(uint8_t state[4][4]) {
    for(int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            state[i][j] = sbox[state[i][j]];
}

void AESImpl::shiftRows(uint8_t state[4][4]) {
    uint8_t temp;
    
    temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    temp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = temp;
}

void AESImpl::mixColumns(uint8_t state[4][4]) {
    for(int i = 0; i < 4; i++) {
        uint8_t a[4];
        uint8_t b[4];
        for(int j = 0; j < 4; j++) {
            a[j] = state[j][i];
            b[j] = (state[j][i] << 1) ^ (0x1B & -(state[j][i] >> 7));
        }
        state[0][i] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
        state[1][i] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
        state[2][i] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
        state[3][i] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
    }
}

void AESImpl::encryptBlock(uint8_t in[16], uint8_t out[16]) {
    uint8_t state[4][4];

    for(int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            state[j][i] = in[i*4 + j];

    addRoundKey(state, &roundKey[0]);

    for(int round = 1; round < NR; round++) {
        subBytes(state);
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, &roundKey[round*4]);
    }

    subBytes(state);
    shiftRows(state);
    addRoundKey(state, &roundKey[NR*4]);

    for(int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            out[i*4 + j] = state[j][i];
}

std::vector<uint8_t> AESImpl::encrypt(const std::vector<uint8_t>& data) {
    size_t padded_size = ((data.size() + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
    std::vector<uint8_t> padded_data(padded_size, padded_size - data.size());
    std::copy(data.begin(), data.end(), padded_data.begin());

    std::vector<uint8_t> encrypted(padded_size);
    for(size_t i = 0; i < padded_size; i += BLOCK_SIZE) {
        encryptBlock(&padded_data[i], &encrypted[i]);
    }

    return encrypted;
}

}}} // namespace io::newgrounds::crypto 